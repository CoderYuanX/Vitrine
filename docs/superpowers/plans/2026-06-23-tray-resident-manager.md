...
**Files:**
- Create: `manager/tray.py`, `manager/assets/managewidgets-connected.svg`, `manager/assets/managewidgets-disconnected.svg`
...
- [ ] **Step 1: 写图标资源(两个极简 SVG 圆点)**

`manager/assets/managewidgets-connected.svg`:
```xml
...
`manager/assets/managewidgets-disconnected.svg`:
```xml
<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22">
...
_ICON_CONNECTED = str(_ASSETS / "managewidgets-connected.svg")
_ICON_DISCONNECTED = str(_ASSETS / "managewidgets-disconnected.svg")
...
class TrayIndicator:
    ...
    def __init__(self, *, on_toggle_window, on_start_core, on_stop_core,
        ...
        # AppIndicator 接受绝对路径作为图标名,直接用 svg 文件
        ...
        item_start = Gtk.MenuItem(label="启动底座")
        item_start.connect("activate", lambda *_: on_start_core())
        menu.append(item_start)
        item_stop = Gtk.MenuItem(label="停止底座")
        item_stop.connect("activate", lambda *_: on_stop_core())
...
- Test: `tests/test_manager_smoke.py`(追加 overview 自启同步用例)
...
- [ ] **Step 1: 给 OverviewPage 加可同步的自启开关**
...
- [ ] **Step 2: 写追加冒烟测试到 `tests/test_manager_smoke.py`**

在文件末尾追加(验证概览自启同步不回环;无显示则随既有 `pytestmark` skip):
```python
...
Run: `.venv/bin/python -m pytest tests/test_manager_smoke.py -q -k overview_autostart_sync`
Expected: FAIL(`OverviewPage` 无 `set_autostart_active`)
...
- [ ] **Step 5: 运行确认通过(冒烟 + 全量回归)**

Run: `.venv/bin/python -m pytest tests/test_manager_smoke.py -v`
Expected: PASS(含新 `test_overview_autostart_sync_no_feedback_loop`)
Run: `.venv/bin/python -m pytest -q`
Expected: PASS(全过;无显示环境下 GUI 用例 SKIP)
...
```bash
git add manager/app.py manager/pages/overview.py tests/test_manager_smoke.py
...
- **测试边界**:T1 纯单测;T2/T3 的 GUI/托盘用例在无显示或无 AppIndicator 时 SKIP;托盘是否真正出现在 dock、关窗对话框、点图标唤出由 T3 Step6 人工 E2E 验收。
