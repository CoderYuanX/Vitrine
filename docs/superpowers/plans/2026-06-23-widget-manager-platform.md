...
- Produces: `Hub.handle` 新增处理 `hello`(鉴权)、`set_provider`、`set_interval`、`shutdown`;未鉴权连接除 `hello` 外一律 `unauthorized` 且 `close=True`;`set_provider` 改 `self._enabled[pid]` **并同步 `self._config.providers_enabled[pid]`**;`set_interval` 改 `self._config.intervals[topic]`;两者**改完即调 `self._persist()`(持久化)**并 `broadcast_status`(set_interval 另带 `repoll`+`reset_timer`)
