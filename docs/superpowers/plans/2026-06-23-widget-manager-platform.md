...
.venv/bin/pip install --quiet tomli-w
.venv/bin/python -c "import gi, psutil, websockets, pytest, tomllib, tomli_w; print('deps OK')"
```
Expected: 末行打印 `deps OK`

- [ ] **Step 2: 写 `.gitignore` 与 `pyproject.toml`**
...
[tool.setuptools]
packages = ["core", "core.providers", "manager", "manager.pages"]
...
- Produces:
  ...
  - `cmdline_is_core(cmdline: str) -> bool`:判断 `/proc/<pid>/cmdline`(NUL 已转空格)是否属于本程序——`return "managewidgets-core" in cmdline`
...
def cmdline_is_core(cmdline: str) -> bool:
    return "managewidgets-core" in cmdline
...
- Produces:
  ...
  - `@dataclass class Reply`: `direct: list[dict] = []`(回给本连接的消息)、`broadcast_status: bool = False`、`repoll: list[str] = []`(需立即重采的 topic)、`reset_timer: list[str] = []`、`shutdown: bool = False`、`close: bool = False`、`status_request_id: str | None = None`(置位时 server 据该 id 给请求方回完整 status)
  - `class Hub`:
    - `__init__(self, providers: list[Provider], config: Config, token: str)`
...
class Hub:
    def __init__(self, providers: list[Provider], config: Config, token: str):
...
- Produces: `Hub.handle` 新增处理 `hello`(鉴权)、`set_provider`、`set_interval`、`shutdown`;未鉴权连接除 `hello` 外一律 `unauthorized` 且 `close=True`;`set_provider` 改 `self._enabled[pid]`;`set_interval` 改 `self._config.intervals[topic]` 并 `repoll`+`reset_timer`+`broadcast_status`
...
```python
    def handle(self, conn: Conn, msg: dict) -> Reply:
        ...
        if action == "set_provider":
            ...
            self._enabled[pid] = bool(msg.get("enabled", True))
...
- Produces:
  - `class CoreServer`:
    - `__init__(self, hub: Hub, host="127.0.0.1", port=0, heartbeat=2.0)`(`heartbeat`=status 广播周期秒,≤0 关闭)
    - `async def serve(self) -> None`(启动 WS 服务 + 各 active topic 的调度任务 + status 心跳,阻塞直到 `stop()`)
...
- Produces:
  - `def build_hub(config, token) -> Hub`(组装 system+time provider)
  - `def main(argv=None) -> int`:① 取实例锁,失败→打印已有端口、返回 3;② 删除残留 runtime;③ 加载配置;④ 生成 token、起 `CoreServer`(端口取配置,占用顺延由 OS/重试处理);⑤ 写 runtime(pid/port/token/started_at/version);⑥ 注册 SIGTERM/SIGINT → 优雅停;⑦ 退出时 `remove_runtime`。返回 0
...
> 注:测试用 `python -m core` 起进程,故 `pid_is_core` 的 cmdline 校验在真实安装下应识别 `managewidgets-core`;此处入口身份不影响本组用例(只验证 runtime/锁/信号)。
...
def build_hub(config, token: str) -> Hub:
    return Hub([SystemProvider(), TimeProvider()], config, token)
...
def main(argv=None) -> int:
    ...
    config, _notices = load_config(default_config_path())
    token = generate_token()
    server = CoreServer(build_hub(config, token), host="127.0.0.1", port=config.port)
...
> 端口顺延说明:`config.port` 被占用时,本版让 `CoreServer` 绑定失败抛错;最简稳妥做法是在 `CoreServer.serve` 里对端口做"占用则 +1 重试至多 N 次"。实现者在 Task 7 的 `serve` 绑定处加一个 try/except 重试小循环(从 `self._port` 起,0 表示交给 OS 选)。若 Task 7 已用 `port=0` 交给 OS,则配置端口可作首选、失败回退 0。本计划默认:`__main__` 传 `config.port`;`serve` 绑定失败时回退 `port=0` 再绑一次。请在 Task 7 `serve` 中补这层回退后再做本 task。

- [ ] **Step 4: 回补 Task 7 `serve` 的端口回退,然后运行确认通过**

在 `core/server.py` 的 `serve()` 里,把 `websockets.serve(self._handler, self._host, self._port)` 改为带回退:

```python
        try:
            server_cm = websockets.serve(self._handler, self._host, self._port)
            server = await server_cm.__aenter__()
        except OSError:
            server_cm = websockets.serve(self._handler, self._host, 0)   # 占用→交给 OS 选端口
            server = await server_cm.__aenter__()
        try:
            self._server = server
            self._actual_port = server.sockets[0].getsockname()[1]
...
Expected: PASS（main_cli 3 passed,且回归 integration 仍全过）

- [ ] **Step 5: 提交**

```bash
git add core/__main__.py core/server.py tests/test_main_cli.py
git commit -m "feat(core): 底座入口 实例锁/runtime/优雅停止 + 端口回退"
```
...
- Produces:
  ...
  - `pages/overview.py`:`class OverviewPage(Gtk.Box)`:显示端口/客户端数/运行时长/连接状态;"启动底座""停止底座"按钮(启动=`subprocess.Popen([sys.executable,"-m","core"])`;停止=`client.send({"action":"shutdown"})`);`update(status_dict)`;`set_connection(state)`;渲染 `core.notices`
...
```
确认:数据源页 CPU/内存/时间实时跳动;改 `system.cpu` 间隔推送频率变化;关 system 开关其 topic 停更而 time 不受影响;关面板重开能自动重连;`core.json` 权限为 `600`。
...
- **覆盖**:provider 框架/system/time(T1-2)、配置+损坏回退(T3)、runtime+token+flock(T4)、协议+鉴权+校验(T5-6)、传输+调度+立即重采(T7)、入口+实例锁+优雅停+端口回退(T8)、发现+WS 客户端+重连+回包关联(T9)、GTK3 三页+按 provider 分组+notices(T10)、autostart(T11)。spec §3-§9 各节均有对应 task。
- **DoD 对齐**:§9 的 11 条验收分散在 T7(数据实时/改间隔/停 provider)、T8(runtime/token/0600/二实例/SIGTERM)、T9-T10(自动重连/一键启动)、手动 E2E(T10 Step5)。
