...
- **权限分级(关键安全约束,现在就钉死)**:本版这个 token 是**全控 token**——能 `shutdown`、`set_provider`、`set_interval`,**仅作为管理面板使用的管理凭证,不注入小组件运行时**。
  - 它**不防同一用户下的其他进程**(`core.json` 同用户可读,该边界已在上文接受);它的作用是划清"管理通道 vs 小组件数据通道"的权限分界,以及挡住**其他用户**(`0600`)。
